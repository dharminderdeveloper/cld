Mastter Service - WordPress Plugin (v1.0)
Author: Dharminder Singh

Installation:
1. Upload the 'mastter-service' folder to wp-content/plugins/
2. Activate the plugin via WordPress admin Plugins screen.
3. Place the shortcode [mastter_service_form] in any page/post where you want the modal/form markup to be available.
   (The modal is initially display:none — you should provide a trigger button on the page that calls showModal())

Files:
- mastter-service.php             (main plugin bootstrap)
- assets/css/frontend.css         (form styling)
- assets/js/frontend.js           (form logic, uses mastterServiceConf.post_url & mastterServiceConf.nonce)
...
- Daily logs are stored in wp-content/plugins/mastter-service/logs/YYYY-MM-DD.json.
- Ensure PHP process can write to plugin folder for logs (or change logs path).
- The JS expects mastterServiceConf.post_url and mastterServiceConf.nonce (localized from PHP).
